...
$UserReportFile = $UsrName + "_InstalledSoftware.dat"
#$NetworkLogDir = "c:\temp\"
...
if (!(Test-Path -Path $NetworkLogDir )) {
    New-Item -ItemType directory -Path $NetworkLogDir | Out-Null
    }

If (Test-Path -Path $($NetworkLogDir + $UserReportFile)) {
        Remove-Item -Path $($NetworkLogDir + $UserReportFile)
    }

$InstalledPrograms = Get-RemoteProgram -Property installdate -LastAccessTime | Sort-Object ProgramName | Select-Object  ComputerName, @{Name='UserName';Expression={$UsrName}}, ProgramName, installdate, LastAccessTime

Write-Host "Working... 25%"

$InstalledPrograms | Export-Csv -NoType $($NetworkLogDir + $UserReportFile) -Encoding UTF8

Copy-Item $($NetworkLogDir + $UserReportFile) "c:\temp\" -Force
